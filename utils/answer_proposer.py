...
def propose_float_answer(answer,keep_non_negative=True,N_answers=4,range_symbol='-'):  
    ...
    if keep_non_negative:
        ...
        order = len(str(abs(int(answer))));
        xorder = max(1,int(abs(int(answer))/10**order));
        if order > 2:
            steps = list( range( 10**(order-2),3*xorder*10**(order-1),10**(order-2) ) )
            if len(steps) == 0:
                chosen_step = min_step;
...
if __name__ == '__main__':
    ...
    print(propose_integer_answer(1756,keep_non_negative=True))
    print(propose_float_answer(11.7,keep_non_negative=False))